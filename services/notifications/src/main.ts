 ...
 *
 * Port:    3012  (configurable via PORT env var)
 * Prefix:  /api/v1
 *
 * Architecture:
 *   - Consumes platform events via internal EventBridge-to-HTTP endpoints
 *   - Sends transactional emails via AWS SES
 *   - Persists Notification records with PENDING → SENT / FAILED status
 *   - All event handlers are idempotent — duplicate delivery is safe
 */
